...
- Treat `agent-readiness-scoring`, `ralph-loop`, `ralph-sprint`, `ralph-status`, and `ralph-review` as generated flagship packages. Change their canonical standalone repositories, then use `python3 scripts/sync_flagship_skills.py update --skill <name> --source-root <path> --apply`; never hand-edit their collection copies.
