...
SHARED_FILES = {
    ...
    "lib/read-config.py": TEMPLATES / "shared" / "read-config.py",
...
def render_config(arguments: argparse.Namespace) -> str:
    ...
    lines = [
        "# Generated by the ralph-loop skill. Parsed as data; shell code is rejected."
    ]
...
def parse_config(path: Path) -> dict[str, str]:
    if path.is_symlink() or not path.is_file():
        raise RalphError(f"Unsafe or missing config: {path}")
    if path.stat().st_size > 128 * 1024:
        raise RalphError(f"Config exceeds 131072 bytes: {path}")
    values: dict[str, str] = {}
    for line_number, raw in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
        ...
        if not re.fullmatch(r"[A-Z][A-Z0-9_]*", key):
            raise RalphError(f"config.env:{line_number} has invalid key {key!r}")
        if key in values:
            raise RalphError(f"config.env:{line_number} repeats {key}")
        try:
            parsed = shlex.split(raw_value, comments=False, posix=True)
        except ValueError as error:
            raise RalphError(f"config.env:{line_number}: {error}") from error
        if len(parsed) > 1:
            raise RalphError(
                f"config.env:{line_number} values with spaces must use shell quoting"
            )
